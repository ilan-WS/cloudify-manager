...
class ResourceManager(object):
    ...
    def delete_deployment_environment(self, deployment_id,
        ...
        if self._any_running_executions(executions):
            ...
            raise manager_exceptions.DependentExistsError(
                f"Can't delete deployment {deployment_ids} - There are "
    ...
    def _check_for_active_executions(self, execution, force, queue):
        should_queue = False
        ...
        if not force:
            ...
            # run when possible
            if len(running) > 0 and (queue or execution.scheduled_for):
                return True
            if len(running) > 0:
                raise manager_exceptions.ExistingRunningExecutionError(
